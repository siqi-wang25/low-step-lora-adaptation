...
def main(args):
    ...
    if args.dataset_name is not None:
        ...
        # If caption column does not exist, auto-build it from image filename
        if "train" in dataset:
            train_cols = dataset["train"].column_names
            if args.caption_column not in train_cols:
                def add_caption_from_filename(example):
                    img = example["image"]
                    fname = os.path.basename(img.filename) if getattr(img, "filename", None) else "untitled"
                    stem = os.path.splitext(fname)[0]
                    # optional cleanup
                    stem = stem.replace("_", " ").replace("-", " ")
                    example[args.caption_column] = stem
                    return example

                dataset["train"] = dataset["train"].map(add_caption_from_filename)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # Save LoRA-only snapshots instead of full training state
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        # keep only folders like lora_step_XXX
                        if args.checkpoints_total_limit is not None:
                            checkpoints = os.listdir(args.output_dir)
                            checkpoints = [d for d in checkpoints if d.startswith("lora_step_")]
                            ...
                            if len(checkpoints) >= args.checkpoints_total_limit:
                                ...
                                logger.info(
                                    f"{len(checkpoints)} LoRA snapshots already exist, removing {len(removing_checkpoints)} snapshots"
                                )
                                logger.info(f"removing LoRA snapshots: {', '.join(removing_checkpoints)}")

                                for removing_checkpoint in removing_checkpoints:
                                    removing_checkpoint = os.path.join(args.output_dir, removing_checkpoint)
                                    shutil.rmtree(removing_checkpoint)

                        save_path = os.path.join(args.output_dir, f"lora_step_{global_step}")
                        ...
                        logger.info(f"Saved LoRA-only weights to {save_path}")
